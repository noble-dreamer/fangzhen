"""Stream-solve Dataset A shell cases and export CSV without saved MPH files."""
...
OUTPUT_ROOT = Path(r'D:\lab_ultr\fz\simple\output\streaming_dataset_a_shell')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Solve Dataset A simple shell cases, export CSV immediately, and do not save solved MPH files.'
    )
    ...
    parser.add_argument('--healthy-sample-id', default='dataset_a_shell_healthy')
...
def configure_a() -> None:
    shell.MODEL_FAMILY = 'Dataset A simple shell streaming solve/export'
    ...
    try:
        ...
        if not args.only_healthy:
            ...
            for sample_id in range(args.start_id, args.start_id + args.samples):
                ...
                name = f'dataset_a_shell_sample_{sample_id:04d}'
                result = streaming.solve_export_sample(
                    client=client,
                    dataset='A',
                    sample_id=name,
